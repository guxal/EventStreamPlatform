...
export class ProjectFactsGeneratorService {
  generate(summary: ProjectGoldSummary): DetectedFact[] {
    ...
    if ((summary.blockedTraffic.blockedInstallRate ?? 0) >= 0.1) add(FactType.PROJECT_HIGH_BLOCKED_TRAFFIC_RATE, Severity.WARNING, 0.92, { blockedInstallRate: summary.blockedTraffic.blockedInstallRate, blockedInstalls: summary.blockedTraffic.blockedInstalls }, 'Review source quality and fraud settings before scaling sources with high blocked install rates.');
    if ((summary.blockedTraffic.blockedEventRate ?? 0) >= 0.1) add(FactType.PROJECT_HIGH_BLOCKED_IN_APP_EVENTS, Severity.WARNING, 0.92, { blockedEventRate: summary.blockedTraffic.blockedEventRate, blockedInAppEvents: summary.blockedTraffic.blockedInAppEvents }, 'Investigate blocked in-app event sources separately from the valid traffic funnel.');
