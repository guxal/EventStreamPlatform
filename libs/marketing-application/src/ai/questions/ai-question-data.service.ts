...
export class AiQuestionDataService {
  ...
  async getEntityPerformance(input: AiQuestionDataInput) { const gold = await this.getProjectGoldSummary(input); return gold ? (gold as any).mediaSourceQuality : this.sourceIsAppsFlyer(input) ? this.appsFlyerEventsRepository.getMediaSources(input.projectId) : []; }
