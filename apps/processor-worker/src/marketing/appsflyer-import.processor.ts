...
import { AiOutputOrchestratorService, ContextBuilderService, SemanticBuilderService } from '@metrics-platform/marketing-application';
import {
...
export class AppsFlyerImportProcessor implements OnModuleInit {
  ...
  async process(payload: MarketingImportProcessJobPayload): Promise<void> {
    ...
    try {
      ...
      await this.runAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.PROCESS_REQUEST_VALIDATION, 'AppsFlyerImportProcessor.validatePayload', async () => ({ valid: true }));

      await this.runAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.BRONZE_PROFILING, 'RawImportFileRepository/DataImportRepository.updateStatus', async () => {
        ...
        return { rawFileStatus: RawFileStatus.PROCESSING, dataImportStatus: ImportStatus.PROCESSING_BRONZE };
      });
      ...
      const streamResult = await this.runAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.BRONZE_PROFILING, 'ObjectStorageService.getObjectStream', async () => {
        const object = await this.objectStorageService.getObjectStream({ bucketName: payload.bucket || rawFile.bucket, objectKey: payload.objectKey || rawFile.objectKey });
        return { result: object, summary: { storageUri: rawFile.storageUri, bucket: payload.bucket || rawFile.bucket, objectKey: payload.objectKey || rawFile.objectKey } };
      });

      await this.dataImportRepository.updateStatus(payload.projectId, payload.dataImportId, ImportStatus.PROCESSING_SILVER);
      const result = await this.runAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.SILVER_PARSING, 'AppsFlyerProcessingPipelineService.execute', async () => {
      ...
      await this.runAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.SILVER_PERSISTENCE, 'ClickHouseMarketingRepository.insertMarketingEvents + MarketingEntityRepository.upsertAppsFlyerDimensions', async () => {
        ...
        return { eventsStored: result.events.length, dimensionsUpserted: true };
      });
      ...
      const semanticResult = await this.runOptionalAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.SEMANTIC_BUILDING, 'SemanticBuilderService.buildForImport', async () => {
        ...
        return { result: buildResult, summary: { entitiesCreated: buildResult.entitiesCreated, relationshipsCreated: buildResult.relationshipsCreated, factsLinked: buildResult.factsLinked, warnings: buildResult.warnings } };
      });

      const contextResult = await this.runOptionalAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.CONTEXT_BUILDING, 'ContextBuilderService.buildForImport', async () => {
        ...
        return { result: buildResult, summary: { contextObjectsCreated: buildResult.contextObjectsCreated, warnings: buildResult.warnings } };
      });
      ...
      await this.runAuditedStep(auditRunId, payload, AppsFlyerProcessingErrorStage.AI_OUTPUT_GENERATION, 'AiOutputOrchestratorService.generateAndStore*', async () => {
        const aiContextExtras = {
          kpis: result.kpis as unknown as Record<string, unknown>,
          warnings: [...(result.warnings ?? []), ...(semanticResult?.warnings ?? []), ...(contextResult?.warnings ?? [])],
        ...
        const recommendations = await this.aiOutputOrchestrator.generateAndStoreRecommendations(payload.projectId, result.facts, aiContextExtras);
        const report = await this.aiOutputOrchestrator.generateAndStoreReport(payload.projectId, result.facts, aiContextExtras);
        return { recommendationsStored: recommendations.length, reportStored: Boolean(report?.id), reportId: report?.id, semanticContextIncluded: Boolean(semanticResult || contextResult) };
      });
      ...
      await this.processAuditRepository.completeRun(auditRunId, { eventsProcessed: result.events.length, factsGenerated: result.facts.length, status: ImportStatus.COMPLETED });
    } catch (error) {
      ...
      await Promise.allSettled([
        ...
        auditRunId ? this.processAuditRepository.failRun(auditRunId, message, { ...summary, eventsProcessed: finalEventsCount, factsGenerated: finalFactsCount }) : Promise.resolve(),
      ]);
      throw error;
    }
  }
  ...
  private async runOptionalAuditedStep<T>(
    ...
    serviceName: string,
    action: () => Promise<T | { result: T; summary?: Record<string, unknown> }> | T | { result: T; summary?: Record<string, unknown> },
  ): Promise<T | null> {
    try {
      return await this.runAuditedStep(runId, payload, stage, serviceName, action);
    } catch (error) {
  ...
  private async runAuditedStep<T>(
    ...
    const step = await this.processAuditRepository.startStep({
      ...
      stage,
      serviceName,
      inputSummary: { correlationId: payload.correlationId, source: payload.source, reportType: payload.reportType },
    });
    try {
  ...
  private inferErrorStage(error: unknown): AppsFlyerProcessingErrorStage {
    ...
    if (message.includes('Semantic')) return AppsFlyerProcessingErrorStage.SEMANTIC_BUILDING;
    if (message.includes('Context')) return AppsFlyerProcessingErrorStage.CONTEXT_BUILDING;
